...
convert_modes(sphere_coeffs_par.', sphere_coeffs_perp.')
...
function convert_modes(sphere_modes_par, sphere_modes_per)


if size(sphere_modes_par, 2) > 1
...
% Get Parallel Modes
    for idx_sims = 1:len_sims
        for idx_lda = 1:len_lda
            dummy = sphere_modes_par{idx_lda, idx_sims};
            orig = dummy(1); % Grab the sphere at the origin data
            
            for n = 1:orig.Nsphere_order
                ...
                Apar{idx_sims, idx_lda, n} = A_0_n;
                Bpar{idx_sims, idx_lda, n} = B_0_n;
                for m = 1:n
                    dummy = A_m_n(m)+((-1).^(m)).*A_neg_m_n(m);
                    Apar{idx_sims, idx_lda, n} =...
                        [Apar{idx_sims, idx_lda, n}, dummy];
                    dummy = B_m_n(m)+((-1).^(m+1)).*B_neg_m_n(m);
                    Bpar{idx_sims, idx_lda, n} =...
                        [Bpar{idx_sims, idx_lda, n}, 2.*dummy];
                end
...
% Get Perpendicular Modes
    for idx_sims = 1:len_sims
        for idx_lda = 1:len_lda
            dummy = sphere_modes_per{idx_lda, idx_sims};
            orig = dummy(1); % Grab the sphere at the origin data
            
            for n = 1:orig.Nsphere_order
                modes_a_per = orig.a_te{n};
                modes_b_per = orig.a_tm{n};
                ...
                for idx_dummy = 1:length(m)
                    if m(idx_dummy)<0
                        A_neg_m_n = [A_neg_m_n,  modes_a_per(idx_dummy)];
                        B_neg_m_n = [B_neg_m_n,  modes_b_per(idx_dummy)];
                    elseif m(idx_dummy)>0
                        A_m_n = [A_m_n, modes_a_per(idx_dummy)];
                        B_m_n = [B_m_n, modes_b_per(idx_dummy)];
                    elseif m(idx_dummy) == 0
                        A_0_n = modes_a_per(idx_dummy);
                        B_0_n = modes_b_per(idx_dummy);
                    end
                end
                    
     
                Aper{idx_sims, idx_lda, n} = A_0_n;
                Bper{idx_sims, idx_lda, n} = B_0_n;
                for m = 1:n
                    dummy = A_m_n(m)+((-1).^(m+1)).*A_neg_m_n(m);
                    Aper{idx_sims, idx_lda, n} =...
                        [Aper{idx_sims, idx_lda, n}, dummy];
                    dummy = B_m_n(m)+((-1).^(m)).*B_neg_m_n(m);
                    Bper{idx_sims, idx_lda, n} =...
                        [Bper{idx_sims, idx_lda, n}, 2.*dummy];
                end
