        ...
        for idx_lda = 1:len_lda
            ...
            [A,B,Ax,Bx] = ...
                unpack_modes(sphere_modes_par{idx_lda,idx_sims},idx_n,...
                                polarization);
            Apar(idx_sims, idx_lda, idx_n, :) = {A};
            Bpar(idx_sims, idx_lda, idx_n,:) = {B};
            Axpar(idx_sims, idx_lda, idx_n,:) = {Ax};
            Bxpar(idx_sims, idx_lda, idx_n,:) = {Bx};
            end
        end
    end
        ...
        for idx_lda = 1:len_lda
            ...
            [A,B,Ax,Bx] = ...
                unpack_modes(sphere_modes_per{idx_lda,idx_sims},idx_n,...
                                polarization);
            Aper{idx_sims, idx_lda, idx_n, :} = {A};
            Bper{idx_sims, idx_lda, idx_n, :} = {B};
            Axper{idx_sims, idx_lda, idx_n, :} = {Ax};
            Bxper{idx_sims, idx_lda, idx_n, :} = {Bx};
            end
        end
    end
    
    Acell = cat(1, Apar, Aper);
    Bcell = cat(1, Bpar, Bper);
    Axcell = cat(1, Axpar, Axper);
    Bxcell = cat(1, Bxpar, Bxper);
    
    
    % Reshape the matrix. 
    
    A = NaN(len_sims, len_lda, Max_Order,Max_Order+1);
    B = NaN(len_sims, len_lda, Max_Order,Max_Order+1);
    Ax = NaN(len_sims, len_lda, Max_Order,Max_Order+1);
    Bx = NaN(len_sims, len_lda, Max_Order,Max_Order+1);
